...
The production origin is `https://joytoy.binscode.site`. Deployment uses Cloudflare Pages static assets only and is designed to remain within the free plan. The build fails if it approaches the free Pages file-count or per-file limits. Product pages prioritize a full-width gallery with collapsible specifications; card thumbnails and gallery preview rails use higher-quality WebP derivatives generated by `web/scripts/prepare_catalog.py`. See [the restructuring plan](.planning/RESTRUCTURE_CATALOG_SITE.md) for deployment, DNS, cost, and rollback details.
